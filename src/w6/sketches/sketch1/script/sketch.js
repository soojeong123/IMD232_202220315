...
class Ball {
  ...
  applyForce(force) {
    const calcedAcc = p5.Vector.div(force, this.mass); //복사를 해서 값을 갖는거
    // const calcedAcc = force.div(this.mass) ; //위랑 달리 값 유지가 안됨?
  ...
  update() {
    ...
    // this.acc.set(0,0); //0으로 만드는 다른 방법
    // this.acc.setMag(0,0); //0으로 만드는 다른 방법
    this.acc.mult(0);
  }
